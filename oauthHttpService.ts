class OAuthHttpService implements oauth.IOAuthHttpService {
    ...
    public constructor(
        ...
        private _config: oauth.IOAuthConfig 
    ) {
    }

    public register(email: string, password: string): any {
        return this._qService((resolve: ng.IQResolveReject<oauth.IAuthenticateResponse>, reject: ng.IQResolveReject<any>) => {
            return this._httpService<oauth.ITokenPayload>({
                method: "POST", 
            ...
            .success((data: any, status: number, headers: ng.IHttpHeadersGetter, config: ng.IRequestConfig) => {
                if (data.succeeded) {
                    resolve({
                        success: true,
                        messages: null
                    } as oauth.IAuthenticateResponse);
                } else {
    ...
    public login(email: string, password: string): any {
        return this._qService((resolve: ng.IQResolveReject<oauth.IAuthenticateResponse>, reject: ng.IQResolveReject<any>) => {
            ...
            .success((data: oauth.ITokenPayload, status: number, headers: ng.IHttpHeadersGetter, config: ng.IRequestConfig) => {
                if (data.error) {
                    resolve({
                        success: false,
    ...
    public get token(): string {
        ...
        try {
            const token = JSON.parse(tokenPayloadJson) as oauth.ITokenPayload;
